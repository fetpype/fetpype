...
Existing fetal brain MRI tools typically focus on individual processing steps and require customized code for pre- and post-processing, as well as to connect different modules, making it difficult to reproduce processing results across studies. `Fetpype` addresses these challenges by providing a configurable, containerized, and `Nipype`-driven solution that integrates state-of-the-art fetal MRI processing tools into a cohesive pipeline. By emphasizing reproducibility, extensibility, and ease of use, `Fetpype` lowers the barrier to applying advanced fetal MRI analysis methods and facilitates consistent processing across sites, scanners, and studies.  In doing so, `Fetpype` improves comparability across studies and supports community collaboration by facilitating the dissemination of new image processing methods for clinical applications. The pipeline is publicly available on \href{https://github.com/fetpype/fetpype}{GitHub}.
